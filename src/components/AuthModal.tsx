...
const AuthModal = () => {
  ...
  const handleForgot = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) throw error;
      toast.success('Verification email sent', { description: `Check ${email} for the password reset link.` });
      setMode('login');
    } catch (err: any) {
            ...
            <div className="text-center mb-7">
              ...
              {mode === 'login' ? (
                authPrompt ? (
                  ...
                  <p className="text-xs font-body text-muted-foreground mt-2">Sign in to continue your journey</p>
                )
              ) : (
            ...
            <AnimatePresence mode="wait">
              {mode === 'login' ? (
                <motion.form key="login" onSubmit={handleLogin} className="space-y-3" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  ...
                  <button type="submit" disabled={loading} className="w-full btn-primary py-3.5 text-xs tracking-[0.25em] uppercase font-body font-medium flex items-center justify-center gap-2 disabled:opacity-60">
                    {loading && <Loader2 size={14} className="animate-spin" />}
                    Sign In
                  </button>
                </motion.form>
              ) : (
                <motion.form key="forgot" onSubmit={handleForgot} className="space-y-3" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <FloatingInput id="email-forgot" type="email" label="Email Address" icon={Mail} value={email} onChange={(e: any) => setEmail(e.target.value)} autoComplete="email" />
                  <button type="submit" disabled={loading} className="w-full btn-primary py-3.5 text-xs tracking-[0.25em] uppercase font-body font-medium flex items-center justify-center gap-2 disabled:opacity-60">
                    {loading && <Loader2 size={14} className="animate-spin" />}
                    Send Reset Link
                  </button>
                  <button type="button" onClick={() => setMode('login')} className="w-full flex items-center justify-center gap-2 text-xs font-body text-muted-foreground hover:text-foreground py-2">
